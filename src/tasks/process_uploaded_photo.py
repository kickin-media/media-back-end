...
from crud.photo import delete_photo
...
def task():
    ...
    while True:
        photo_data = db.query(Photo).filter(Photo.upload_processed == False).offset(offset).first()
        ...
        try:
            process_photo(photo_data=photo_data, db=db)
        except Exception as err:
            if 'An error occurred (404) when calling the HeadObject operation: Not Found' in str(err):
                upload_cutoff = datetime.now() - timedelta(seconds=S3_UPLOAD_EXPIRY * 2)
                if photo_data.uploaded_at < upload_cutoff:
                    print("Photo {} has not been uploaded and upload window expired. Removing.".format(photo_data.id))
                    delete_photo(db=db, photo=photo_data)
                else:
                    print("Photo {} has not been uploaded yet. Awaiting upload.".format(photo_data.id))
                    offset += 1
            else:
                raise err
...
def process_photo(photo_data: Photo, db: SessionLocal):
    if photo_data.upload_processed:
        re_process = True
        print("Re-processing upload {}...".format(photo_data.id))
    else:
        re_process = False
        print("Processing upload {}...".format(photo_data.id))

    s3 = boto3.resource('s3')

    if re_process:
        upload_location = "/".join([S3_BUCKET_ORIGINAL_PATH, "{}_o.jpg".format(photo_data.id)])
    else:
        upload_location = "/".join([S3_BUCKET_UPLOAD_PATH, photo_data.secret, "{}.jpg".format(photo_data.id)])

    sizes = {
        'original': {
            'path': '{prefix}/{photo_uid}_o.jpg'.format(photo_uid=photo_data.id, prefix=S3_BUCKET_ORIGINAL_PATH)
        },
        'original_size': {
            'path': '{prefix}/{secret}/{photo_uid}_xl.jpg'.format(photo_uid=photo_data.id, secret=photo_data.secret,
                                                                  prefix=S3_BUCKET_PHOTO_PATH)
        },
        'large': {
            'path': '{prefix}/{secret}/{photo_uid}_l.jpg'.format(photo_uid=photo_data.id, secret=photo_data.secret,
                                                                 prefix=S3_BUCKET_PHOTO_PATH),
            'length': 2048
        },
        'medium': {
            'path': '{prefix}/{secret}/{photo_uid}_m.jpg'.format(photo_uid=photo_data.id, secret=photo_data.secret,
                                                                 prefix=S3_BUCKET_PHOTO_PATH),
            'length': 800
        },
        'small': {
            'path': '{prefix}/{secret}/{photo_uid}_s.jpg'.format(photo_uid=photo_data.id, secret=photo_data.secret,
                                                                 prefix=S3_BUCKET_PHOTO_PATH),
            'length': 400
        }
    }
    ...
    if photo_data.author is not None:
    ...
    # Mark as processed.
    statement = update(Photo).where(Photo.id == photo_data.id).values({
        'upload_processed': True,
        'exif_data': json.dumps(new_image_exif)
    })
