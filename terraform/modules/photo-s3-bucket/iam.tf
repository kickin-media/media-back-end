resource "aws_iam_user" "upload_user" {
  name = "${aws_s3_bucket.photo_s3_bucket.bucket}-upload-user"
}
...
resource "aws_iam_policy" "upload_user_policy" {
  name   = "${aws_s3_bucket.photo_s3_bucket.bucket}-upload-policy"
  policy = jsonencode({
    Version   = "2012-10-17"
    Statement = [
      {
        Effect   = "Allow",
        Resource = "${aws_s3_bucket.photo_s3_bucket.arn}/*"
        Action   = [
          "s3:PutObject",
          "s3:GetObject",
          "s3:HeadObject",
      ...
      {
        Effect   = "Allow",
        Resource = var.sqs_processing_queue_arn
        Action   = [
